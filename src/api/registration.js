...
router.post('/signup', async (req, res) => {
  ...
  const alreadyExistsNickname = await User.findOne({ where: { nickname } }).catch(
    (err) => {
      console.log('Error: ', err);
      res.status(500).json({ message: err });
    }
  ...
  if (alreadyExistsNickname) {
    return res.status(406).json({ message: 'User with nickname already exists!' });
  }
...
router.post('/signup/google', async (req, res) => {
  const { nickname, email, provider } = req.body;
  ...
  if (!alreadyExistsUser) {
    const newUser = new User({ nickname, email, provider });
    const savedUser = await newUser.save().catch((err) => {
      console.log('Error: ', err);
      res.status(500).json({ error: 'Cannot register user at the moment!' });
    });
